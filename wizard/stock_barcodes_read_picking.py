...
class WizStockBarcodesReadPicking(models.TransientModel):
    ...
    total_product_quantity = fields.Float(compute="_compute_total_product")
    ...
    @api.depends("todo_line_ids", "picking_id.move_line_ids.quantity")
    ...
    @api.depends(
        "todo_line_ids", "todo_line_ids.quantity", "picking_id.move_line_ids.quantity"
    )
    def _compute_move_line_ids(self):
        self.move_line_ids = self.picking_id.move_line_ids.filtered("quantity").sorted(
            key=lambda sml: (sml.write_date, sml.create_date), reverse=True
        )

    @api.depends("picking_id.move_line_ids.quantity")
    def _compute_total_product(self):
        self.total_product_uom_qty = 0.0
        self.total_product_quantity = 0.0
        for rec in self:
            ...
            for line in product_moves:
                ...
                rec.total_product_quantity += line.quantity_done
    ...
    def _get_stock_move_lines_todo(self):
        move_lines = self.picking_id.move_line_ids.filtered(
            ...
            and ml.quantity < ml.reserved_qty
        )
        return move_lines
    ...
    def determine_todo_action(self, forced_todo_line=False):
        ...
        self.todo_line_id._compute_quantity()
        ...
        if self.option_group_id.get_option_value("product_qty", "filled_default"):
            self.product_qty = move_line.product_uom_qty - move_line.quantity
        else:
    ...
    def update_fields_after_determine_todo(self, move_line):
        self.picking_product_qty = move_line.quantity
    ...
    def _process_stock_move_line(self):  # noqa: C901
        ...
        for line in lines:
            if line.reserved_uom_qty and len(lines) > 1:
                assigned_qty = min(
                    max(line.reserved_uom_qty - line.quantity, 0.0), available_qty
                )
            else:
            ...
            if (
                ...
                quantity = line.quantity + assigned_qty
            sml_vals.update(
                {
                    "quantity": quantity,
                    "result_package_id": self.result_package_id.id,
            ...
            if line.quantity >= line.reserved_uom_qty:
        ...
        if (
            ...
            for sml in stock_move_lines:
                ...
                move_lines_dic[sml.id] = sml.quantity
